...
const processValidationRequests = async (
  ...
  try {
    ...
    console.log(`[ARN-AGENT] Processing validation requests (chainId=${chainId}, agentId=${agentIdFilter || 'ALL'}, requestHash=${requestHashFilter || 'ALL'})`);
    ...
    console.error('[ARN-AGENT] Error processing validation requests:', error);
    throw error;
  }
};
...
const getAgentValidationsSummary = async (chainId: number, agentId: string): Promise<any> => {
  try {
    ...
    console.error('[ARN-AGENT] Error getting validation summary:', error);
    throw error;
  }
};

import { getD1Database } from './lib/d1-wrapper.js';
import type { Agent, AgentFeedbackRequest } from './lib/db.js';
...
function getA2aPrivateKey() {
  ...
  if (!pem) {
    console.warn('[ARN-AGENT] A2A_ED25519_PRIVATE_KEY_PEM is not set; HTTP signatures will be omitted.');
    return null;
  }
  try {
    return createPrivateKey({ key: pem });
  } catch (e) {
    console.error('[ARN-AGENT] Failed to load Ed25519 private key from A2A_ED25519_PRIVATE_KEY_PEM:', e);
    return null;
  }
}
...
function addHttpSignatureHeaders(c: any, targetUri: string) {
  ...
  try {
    ...
    console.error('[ARN-AGENT] Failed to add HTTP Message Signature headers:', e);
  }
}

/**
 * Simple Hono-based ATP Agent Provider
 * Exposes:
...
app.use('*', async (c, next) => {
  if (c.env && typeof process !== 'undefined') {
    ...
    try {
      ...
      console.warn('[ARN-AGENT] Failed to populate process.env from c.env:', e);
    }
  }
  await next();
});
...
app.use('*', async (c, next) => {
  ...
  if (!domainInfo) {
    const fallbackAgentName = process.env.AGENT_NAME || 'default-arn';
...
app.get('/.well-known/agent-card.json', async (c) => {
  try {
    ...
    // Fetch agent + session package from D1
    let agentData: Agent | null = null;
    let sessionPackageFromDb: any = null;
    try {
      const db = await getD1Database();
      if (db) {
        console.log('[Agent Card] Querying D1 database for agent:', ensName);
        agentData = await db
          .prepare('SELECT * FROM agents WHERE ens_name = ?')
          .bind(ensName)
          .first<Agent>();

        if (agentData) {
          console.log('[Agent Card] Found agent in D1:', {
            agent_name: agentData.agent_name,
            email_domain: agentData.email_domain,
            has_session_package: !!agentData.session_package,
          });

          if (agentData.session_package) {
            try {
              sessionPackageFromDb = JSON.parse(agentData.session_package);
          ...
          console.log('[Agent Card] No agent found in D1 for:', ensName);
        }
      } else {
        console.warn(
    ...
    const description = `${displayName} - ATP Agent`;
    ...
    if (agentData) {
      (agentCard as any).metadata = {
        ...
        agentId,
        chainId: chainId ? Number(chainId) : undefined,
        agentAccount: agentAccount || agentData.agent_account,
        did: agentDid,
      } as any;
    } else {
      (agentCard as any).metadata = {
...
async function handleA2aRequest(c: any) {
  ...
  try {
    ...
    console.log('[ARN-AGENT] Body:', JSON.stringify(body, null, 2));
  } catch (e) {
    console.error('[ARN-AGENT] Failed to parse JSON body:', e);
  ...
  try {
    ...
    try {
      const client = await getAgenticTrustClient();
      console.log('[ARN-AGENT] Resolving agent from ENS:', ensName);
      const agent = await client.getAgentByName(ensName);

      if (agent) {
        console.log('[ARN-AGENT] Agent object keys:', Object.keys(agent));
        console.log('[ARN-AGENT] Agent summary:', {
          agentId: (agent as any).agentId,
          agentAccount:
            (agent as any).agentAccount ??
            (agent as any).account ??
        ...
        console.log('[ARN-AGENT] Extracted agent info:', { agentId, agentAccount, chainId });
      } else {
        console.warn('[ARN-AGENT] Agent not found for ENS:', ensName);
      }
    } catch (error) {
      console.warn('[ARN-AGENT] Could not resolve agent from ENS, using defaults:', error);
      console.warn('[ARN-AGENT] Error details:', error instanceof Error ? error.stack : error);
    }

    console.log('[ARN-AGENT] Processing A2A for agent:', { agentName, ensName, agentId, agentAccount });
    ...
    if (!skillId && (!fromAgentId || !toAgentId)) {
      console.warn(
        '[ARN-AGENT] A2A request without fromAgentId/toAgentId and no skillId – treating as generic handshake/echo',
      );
    }
    ...
    if (auth) {
      ...
      //console.log('[ARN-AGENT] Client authenticated:', {
      //  did: auth.did,
      //  kid: auth.kid,
      ...
      console.warn('[ARN-AGENT] A2A request received without authentication');
    }

    console.log('[ARN-AGENT] Received A2A message:', {
      fromAgentId,
      toAgentId,
      message,
      payload,
    ...
    if (skillId === 'agent.feedback.requestAuth') {
      ...
      console.log('[ARN-AGENT] ========================================');
      console.log('[ARN-AGENT] Feedback Auth Request Details:', {
        skillId,
    ...
    if (skillId === 'general_movie_chat') {
      ...
      try {
        ...
        try {
          ...
          console.log('[ARN-AGENT] Loading session package from database for:', ensName);
          const agent = await db
            .prepare('SELECT session_package FROM agents WHERE ens_name = ?')
            .bind(ensName)
            .first<{ session_package: string | null }>();
          
          if (agent?.session_package) {
            sessionPackage = JSON.parse(agent.session_package);
            console.log('[ARN-AGENT] Session package loaded from database');
            console.log('[ARN-AGENT] Session package has secret:', !!(sessionPackage as any)?.secret);
            console.log('[ARN-AGENT] Session package agentId:', (sessionPackage as any)?.agentId);
          } else {
            throw new Error(`No session package found in database for: ${ensName}. Please store the session package in the agents table.`);
          }
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
          console.error('[ARN-AGENT] Failed to load session package from database:', errorMessage);
        ...
        console.log('[ARN-AGENT] Calling handleFeedbackAuthRequest with sessionPackage:', {
        ...
        console.log('[ARN-AGENT] Feedback auth created successfully:', {
        ...
        try {
          ...
          if (db) {
            ...
            console.log('[ARN-AGENT] Starting feedback request update logic:', {
              feedbackRequestId,
            ...
            if (feedbackRequestId !== undefined && feedbackRequestId !== null) {
              console.log('[ARN-AGENT] FeedbackRequestId provided, attempting to update by ID:', feedbackRequestId);
              const requestId = typeof feedbackRequestId === 'string' 
                ? parseInt(feedbackRequestId, 10) 
                : Number(feedbackRequestId);
              
              console.log('[ARN-AGENT] Parsed requestId:', requestId, 'isFinite:', Number.isFinite(requestId));
              
              if (Number.isFinite(requestId)) {
                // Verify the record exists and matches the client/agent
                console.log('[ARN-AGENT] Querying for feedback request:', {
                  requestId,
                  clientAddress: clientAddress.toLowerCase(),
                ...
                console.log('[ARN-AGENT] Query result:', existing);
                ...
                if (existing?.id) {
                  ...
                  await db
                    ...
                    .run();
                  console.log('[ARN-AGENT] Updated feedback request by ID with feedback auth:', requestId);
                  updated = true;
                  effectiveRequestId = existing.id;
                  console.log('[ARN-AGENT] Set effectiveRequestId to:', effectiveRequestId);
                } else {
                  console.warn('[ARN-AGENT] Feedback request ID not found or does not match client/agent:', {
                    requestId,
                    clientAddress: clientAddress.toLowerCase(),
                    targetAgentId: String(resolvedAgentId),
                    existing,
                ...
                console.warn('[ARN-AGENT] Invalid requestId (not a number):', requestId);
              }
            } else {
              console.log('[ARN-AGENT] No feedbackRequestId provided in payload');
            }
            
            // If no feedbackRequestId provided, or if the ID lookup failed, use existing logic
            if (!updated) {
              console.log('[ARN-AGENT] No update yet, trying to find existing feedback request by client/agent');
              ...
              console.log('[ARN-AGENT] Existing feedback request lookup result:', existing);
              ...
              if (existing?.id) {
                ...
                await db
                  ...
                  .run();
                console.log('[ARN-AGENT] Updated existing feedback request with feedback auth:', existing.id);
                effectiveRequestId = existing.id;
                console.log('[ARN-AGENT] Set effectiveRequestId to:', effectiveRequestId);
              } else {
                // Create new record
                console.log('[ARN-AGENT] No existing request found, creating new feedback request record');
                const result = await db
                  ...
                  .run();
                console.log('[ARN-AGENT] Created new feedback request record with feedback auth:', result.meta.last_row_id);
                effectiveRequestId = result.meta.last_row_id as number;
                console.log('[ARN-AGENT] Set effectiveRequestId to:', effectiveRequestId);
              }
            }
            
            console.log('[ARN-AGENT] Final effectiveRequestId before message creation:', effectiveRequestId);

            // If we have an effective request id, send a message back to the requester
            if (effectiveRequestId !== undefined) {
              console.log('[ARN-AGENT] ========================================');
              console.log('[ARN-AGENT] Creating notification message for feedback auth approval');
              console.log('[ARN-AGENT] ========================================');
              ...
              console.log('[ARN-AGENT] Message details:', {
                effectiveRequestId,
                clientAddress,
                resolvedAgentId,
                chainId,
              ...
              try {
                console.log('[ARN-AGENT] Inserting message into database...');
                const messageResult = await db
                  ...
                  .bind(
                    ...
                    'Your request to give feedback has been approved. Open the ATP dashboard to submit your feedback for this agent.',
                ...
                console.log('[ARN-AGENT] Message inserted successfully. Message ID:', messageResult.meta.last_row_id);
                console.log('[ARN-AGENT] Created inbox message notifying client of feedback auth:', {
                ...
                console.log('[ARN-AGENT] ========================================');
                console.log('[ARN-AGENT] Notification message creation completed');
                console.log('[ARN-AGENT] ========================================');
              } catch (messageError) {
                console.error('[ARN-AGENT] ========================================');
                console.error('[ARN-AGENT] Failed to create notification message:', messageError);
                console.error('[ARN-AGENT] Error details:', messageError instanceof Error ? messageError.stack : messageError);
                console.error('[ARN-AGENT] ========================================');
                // Don't fail the whole request if message creation fails
              }
            } else {
              console.warn('[ARN-AGENT] No effectiveRequestId available, skipping notification message creation');
              console.warn('[ARN-AGENT] Debug info:', {
                feedbackRequestId,
                updated,
              });
            }
          } else {
            console.warn('[ARN-AGENT] Database not available, cannot save feedback request record');
          }
        } catch (dbError) {
          console.error('[ARN-AGENT] Error saving feedback request record:', dbError);
          // Don't fail the request if database save fails
        }
      } catch (error: any) {
        console.error('[ARN-AGENT] Error creating feedback auth:', error);
      ...
      try {
        ...
        if (!agentIdParam) {
          ...
          console.log('[ARN-AGENT] ========================================');
          console.log('[ARN-AGENT] DETECTED: Validation Respond Request');
          console.log('[ARN-AGENT] ========================================');
          console.log('[ARN-AGENT] Validation Request Details:', {
            skillId,
            agentId: resolvedAgentId,
          ...
          try {
            ...
            if (org?.session_package) {
              ...
              console.log('[ARN-AGENT] Session package loaded from database');
              console.log('[ARN-AGENT] Session package structure:', {
          ...
          console.log('[ARN-AGENT] Processing validation requests via internal validation service');
          console.log('[ARN-AGENT] Agent ID:', resolvedAgentId, 'Chain ID:', resolvedChainId, 'Request Hash:', requestHash || 'ALL');
          ...
          if (successCount > 0 && requestHash) {
            try {
              const db = await getD1Database();
              if (!db) {
                console.warn('[ARN-AGENT] D1 database not available for notification message');
              } else {
                ...
                if (originalMessage && originalMessage.from_agent_did) {
                  ...
                  console.log('[ARN-AGENT] Created notification message for validation response:', {
                    requestHash,
                  ...
                  console.warn('[ARN-AGENT] Could not find original validation request message for requestHash:', requestHash);
                }
              }
            } catch (messageError) {
              console.warn('[ARN-AGENT] Failed to create validation response notification message:', messageError);
              // Don't fail the whole request if message creation fails
            }
          }
        }
      } catch (validationError: any) {
        console.error('[ARN-AGENT] Error processing validation request:', validationError);
      ...
      try {
        ...
        console.log('[ARN-AGENT] Stored feedback request:', {
          clientAddress,
        ...
        try {
          ...
          await messagesDb
            ...
            .run();
          console.log('[ARN-AGENT] Created inbox message for feedback request:', {
            feedbackRequestId,
            clientAddress,
            targetAgentDid,
            targetAgentName,
          ...
          console.warn('[ARN-AGENT] Failed to create inbox message for feedback request:', msgError);
        }
        ...
        console.error('[ARN-AGENT] Error processing feedback request:', error);
      ...
      try {
        ...
        console.log('[ARN-AGENT] Querying feedback requests for client address:', clientAddress);
        ...
        console.log('[ARN-AGENT] Found feedback requests:', feedbackRequests.length);
        ...
        console.error('[ARN-AGENT] Error querying feedback requests:', error);
      ...
      try {
        ...
        console.log('[ARN-AGENT] Querying feedback requests for target agent ID:', targetAgentId);
        ...
        console.log('[ARN-AGENT] Found feedback requests for agent:', feedbackRequests.length);
        ...
        console.error('[ARN-AGENT] Error querying feedback requests by agent:', error);
        responseContent.error = error instanceof Error ? error.message : 'Failed to query feedback requests';
        responseContent.skill = skillId;
        responseContent.success = false;
      }
      ...
      try {
        ...
        console.error('[ARN-AGENT] Error marking feedback request as given:', error);
      ...
      try {
        ...
        try {
          ...
          if (!fromAgentDid && fromAgentName) {
            try {
              ...
              console.warn('[ARN-AGENT] Failed to resolve FROM agent DID from name for inbox message:', e);
            }
            ...
            try {
              ...
              console.warn('[ARN-AGENT] Failed to resolve FROM agent name from DID for inbox message:', e);
            }
          }
          ...
          if (!toClientAddress && (toAgentName || toAgentDid)) {
            try {
              ...
              console.warn('[ARN-AGENT] Failed to resolve TO agent identity for inbox message:', e);
            }
          }
        } catch (resolveError) {
          console.warn('[ARN-AGENT] Failed to resolve agent identity for inbox message:', resolveError);
        }
        ...
        console.error('[ARN-AGENT] Error storing inbox message:', error);
      ...
      try {
        ...
        console.error('[ARN-AGENT] Error querying client messages:', error);
      ...
      try {
        ...
        console.log('[ARN-AGENT] ========================================');
        console.log('[ARN-AGENT] Querying agent messages for DID:', agentDid);
        console.log('[ARN-AGENT] DID length:', agentDid.length);
        console.log('[ARN-AGENT] ========================================');
        ...
        console.log('[ARN-AGENT] Sample DIDs in database:', {
        ...
        console.log('[ARN-AGENT] Query result:', {
          queryDid: agentDid,
        ...
        console.error('[ARN-AGENT] Error querying agent messages:', error);
        responseContent.error = error instanceof Error ? error.message : 'Failed to query messages';
        responseContent.skill = skillId;
        responseContent.success = false;
      }
      ...
      try {
        ...
        console.error('[ARN-AGENT] Error marking message as read:', error);
...
if (isMainModule) {
  ...
  console.log(`[ARN-AGENT] Hono provider listening on port ${port}`);
  serve({
    fetch: app.fetch,
    port
  });
