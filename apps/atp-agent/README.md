...
A2A (Agent-to-Agent) provider server for smart agent communication endpoints. This server enables smart agents to communicate with each other and handle A2A protocol messages. Each smart agent has associated smart agent accounts, smart agent identities, and can run smart agent applications.
...
- Example: `example-arn.8004-agent.eth/.well-known/agent-card.json`

### Path-Based Routing
Access agents via path:
- `/{agent-name}/.well-known/agent-card.json` → Routes to that agent's endpoints
- Example: `/example-arn/.well-known/agent-card.json`
...
AGENT_DESCRIPTION=Description of my smart agent
AGENT_ID=123
AGENT_ADDRESS=0x...

# Provider Configuration
PROVIDER_BASE_URL=http://localhost:3001
PROVIDER_ORGANIZATION=My Smart Agent Provider

# Session Package (from ATP web app)
...
### `organizations`
Stores smart agent information, including session packages, smart agent accounts, and smart agent identities.
...
wrangler d1 execute atp --file=./db/migrations/0001_add_feedback_auth.sql

# Or for local development
wrangler d1 execute atp --local --file=./db/migrations/0001_add_feedback_auth.sql
```
