...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // We need to preserve the original hostname (e.g., gmail-arn.localhost:3000)
    // for routing, but use localhost:PORT for the actual connection
    const urlObj = new URL(a2aEndpoint);
    const originalHost = urlObj.host; // e.g., "gmail-arn.localhost:3000"
    ...
    console.log('[feedback-auth] This should hit arn-agent server on port 3000');
    console.log('[feedback-auth] ========================================');
    console.log('\n\n\n');
    ...
    // to ensure the Host header is set to gmail-arn.localhost:3000 for routing
    ...
    if (urlObj.hostname.includes('.localhost')) {
      ...
      const response = await new Promise<Response>((resolve, reject) => {
        const options = {
          ...
          headers: {
            ...plainHeaders,
            'Host': originalHost, // Set Host header to gmail-arn.localhost:3000 for routing
