 ...
 * Account Service
 * 
 * Provides utilities for managing account abstraction clients for user accounts.
 */
export class AccountService {
  /**
   * Build a counterfactual AA account client for an account using a fixed salt (0x1),
   ...
   */
  static async getCounterfactualAccountClientByAccount(
    eoaAddress: `0x${string}`,
    options?: GetAAAccountClientOptions
  ): Promise<any> {
    const chain = options?.chain || sepolia;

    console.info(" @@@@@@@@@@@@ getting counterfactual account client by account: ", eoaAddress);
...
export const getCounterfactualAccountClientByAccount =
  AccountService.getCounterfactualAccountClientByAccount;

// Legacy export for backwards compatibility
export const IndivService = AccountService;
export const getCounterfactualAccountClientByIndividual = getCounterfactualAccountClientByAccount;
