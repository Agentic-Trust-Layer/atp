...
export default function DashboardPage() {
  ...
  const [agentData, setAgentData] = React.useState<AgentAssociation | null>(null);
  ...
  React.useEffect(() => {
    ...
    async function loadAgentInfo() {
      ...
      if (agentToUse) {
        ...
        // Fetch user profile and agent data from database
        ...
        if (user?.email) {
          try {
            ...
            console.log("[DashboardPage] Fetching user agents for:", user.email);
            const agents = await getUserAgents(user.email);
            console.log("[DashboardPage] User agents result:", agents);
            const primaryAgent = agents.find(agent => agent.is_primary) || agents[0];
            console.log("[DashboardPage] Primary agent:", primaryAgent);
            if (primaryAgent && isMounted) {
              console.log("[DashboardPage] Setting agentData");
              setAgentData(primaryAgent);
            }
          } catch (err) {
      ...
      if (isMounted) {
        setLoading(false);
        //setError(
        //  "ATP agent is not defined. Please complete the standard connection/onboarding flow to register or select an ATP agent."
        //);
      }
    }
    ...
    async function fetchDatabaseInfo() {
      ...
      try {
        ...
        if (profile) {
          ...
          console.log("[DashboardPage] Fetching user agents (secondary)");
        const agents = await getUserAgents(userEmail);
          console.log("[DashboardPage] User agents (secondary) result:", agents);
        const primaryAgent = agents.find(agent => agent.is_primary) || agents[0];
        console.log("[DashboardPage] Primary agent (secondary):", primaryAgent);
        if (primaryAgent) {
          console.log("[DashboardPage] Setting agentData (secondary)");
          setAgentData(primaryAgent);
        }
      } catch (err) {
