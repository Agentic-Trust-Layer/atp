  ...
  const handleNavigateToOrganization = useCallback(
    (agent: Agent) => {
      try {
        ...
        console.error('[AgentsPage] Failed to navigate to agent details:', error);
      }
    },
    [router],
  );
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  if (!web3auth?.provider) {
                    throw new Error('Connect your wallet to register with smart agent');
                  }

                  // Check if user has a default org agent
                  if (!defaultOrgAgent) {
                    throw new Error('No default smart agent found. Please create a smart agent first.');
                  }

                  if (!defaultOrgAgent.agentAccount) {
                    throw new Error('Default smart agent account not found.');
                  }
                  ...
                  if (!defaultAgentId) {
                    throw new Error('Default smart agent ID is required');
                  }
                  const defaultAgentName = defaultOrgAgent.agentName || defaultOrgAgent.ensName || '';
                  if (!defaultAgentName) {
                    throw new Error('Default smart agent name is required');
                  }
                  ...
                  console.error('Failed to register with smart agent:', err);
                  
                  // Check if the error is about an existing validation
                  const errorMessage = err instanceof Error ? err.message : String(err);
                  let userFriendlyMessage = 'Failed to register with smart agent';
                  
                  if (errorMessage.includes('exists') || 
                      errorMessage.includes('0x08c379a0') ||
                      errorMessage.includes('657869737473')) {
                    userFriendlyMessage = 'A validation request for this agent already exists. You may have already registered with this smart agent.';
              ...
              variant="contained"
            >
              {registrationSubmitting ? 'Registering...' : 'Register with Smart Agent'}
            </Button>
          </DialogActions>
        </Dialog>
