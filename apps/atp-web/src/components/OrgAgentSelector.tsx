...
import { type AgentAssociation } from "../app/service/userProfileService";

interface OrgAgentSelectorProps {
  organizations: AgentAssociation[];
  onSelect: (agent: DefaultOrgAgent) => void;
  onCancel?: () => void;
}

/**
 * Component to select a default ATP agent when user has multiple ATP agents
...
export function OrgAgentSelector({ organizations, onSelect, onCancel }: OrgAgentSelectorProps) {
  ...
  const handleSelect = () => {
    ...
    // Convert AgentAssociation to DefaultOrgAgent
    ...
    >
      ...
      >
        <h2 style={{ fontSize: "1.5rem", marginBottom: "1rem", fontWeight: 600 }}>
          Select Default ATP Agent
        </h2>
        <p style={{ marginBottom: "1.5rem", color: "#64748b", fontSize: "0.9rem" }}>
          Your account is associated with multiple ATP agents. Please select which one to use as your default.
          <br />
        </p>
